...
func _heroes() -> Array:
	...
	for h in GameState.heroes.values():
		result.append({
			...
			"career_role": h.get("career_role", h["career"]),
			"career":   h["career"],
			...
			"wound_state": h.get("wound_state", "healthy"),
			"stats":    h.get("stats", {}),
			"wfrp_stats": h.get("wfrp_stats", {}),
			"skill_ids": h.get("skill_ids", []),
			"skill_names": h.get("skill_names", []),
			"starting_trappings": h.get("starting_trappings", []),
			"starting_talents": h.get("starting_talents", []),
